...
V_WAVE = 2.5        # 波速
DT = 0.05           # 时间步长
STEP_SIZE = 0.08    # S 移动步长
MAX_TIME = 15.0     # 最大演示时间
# 障碍物（放在旁边，不挡路，让绿色圆自由波动）
OBSTACLES = []
# ============================================================
# 全局状态
# ============================================================
...
ax1.set_title('上图：绿色圆由小到大波动', fontsize=14, fontweight='bold')
ax1.set_xlabel('X')
ax1.set_ylabel('Y')
...
ax2.set_title('下图：绿色圆逐渐向终点收敛', fontsize=14, fontweight='bold')
ax2.set_xlabel('X')
ax2.set_ylabel('Y')
...
def animate(frame):
    ...
    # 显示绿色圆收敛过程：每隔一定帧数取一个绿色圆
    total_frames = len(frames_data)
    num_display = 12  # 显示的绿色圆数量
    ...
    if O is not None and R is not None and R > 0:
        ...
        green_circles_2.append(patch)
    info_text_2.set_text(
        f"⏱ t={t_curr:.2f}s | 绿色圆历史数: {green_count}\n"
        f"📏 距离终点: {d:.2f} | 收敛中..."
    )
    return (S_point_1, S_point_2, path_line_1, path_line_2,
            info_text_1, info_text_2, S_wave_1, E_wave_1)
