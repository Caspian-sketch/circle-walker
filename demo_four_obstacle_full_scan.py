...
def select_M(O, R, E, beta, obstacles, last_theta, k_r):
    ...
    num_samples = 300
    # 全向搜索，覆盖整个绿色圆
    raw_angles = np.linspace(-np.pi, np.pi, num_samples * 2)
    ...
    search_angles_full = (np.linspace(-np.pi, np.pi, num_samples * 2) + np.pi) % (2 * np.pi) - np.pi
    ...
    for theta in search_angles_full:
        in_forbidden = False
        for low, high in forbidden:
            if low <= high:
                if low <= theta <= high:
        ...
        for r in reversed(radii):
            ...
            if safe:
                ...
                angle_penalty = abs((theta - theta_target + np.pi) % (2 * np.pi) - np.pi)
                score = dist_to_E + angle_penalty * R * 0.5
                if score < best_score:
                    best_score = score
                    best_M = candidate
                break
